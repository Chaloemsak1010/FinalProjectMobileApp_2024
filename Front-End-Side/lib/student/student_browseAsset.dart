...
class BookAsset extends StatefulWidget {
  const BookAsset({super.key});

  @override
  State<BookAsset> createState() => _BookAssetState();
}
...
class _BookAssetState extends State<BookAsset> {
...
// Filter assets based on search query
  ...
  Future<void> checkBorrowing() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      String? token = prefs.getString("token");
      ...
      // API endpoint
      final url = Uri.parse('http://10.0.2.2:3000/student/checkBorrowing');
      ...
      if (response.statusCode == 200) {
        ...
        // safe borowingID
        await prefs.setInt("borrowingID" , data['borowingID']);
        borrowingID = data['borowingID'];
      } else {
        debugPrint("Failed to load data: ${response.statusCode}");
      }
    } catch (error) {
            ...
            child: GridView.builder(
              ...
              itemBuilder: (context, index) {
                return GestureDetector(
                  onTap: widget.assets[index].status != "Available" || widget.borowingID != null
                  // T : mean can not request
                      ? () {
                        _showSnackbar("can not request");
                      }
                      : () {
                          Navigator.push(
                            context,
